...
//     return (
//         <View style={{ width: SIZES.width, zIndex: 1000, backgroundColor: '#fff' }}>
...
//                                                     style={{
//                                                         color: CurrentPageIdContext == arrayItem.pageid ? templateproperties_context.active_iconcolor : templateproperties_context.bottomnav_iconcolor,
//                                                     }}
...
const BottomTabs = (props) => {
    const { lang, langdetect } = useContext(LanguageContext);
    const { templateproperties_context, setVendorsArrgeneralFilterContext, fetchcustomercartQueryContext } = useContext(FetchingContext);
    const { setCurrentPageIdContext, CurrentPageIdContext } = useContext(WebsiteDesignWorkPlaceContext);
    // const [currentpage, setcurrentpage] = useState(props.currentpageprops);
    const [TabexIcons, setTabexIcons] = useState({
        home1: { family: 'AntDesign', name: 'home' },
        ...
        percentage1: { family: 'Feather', name: 'percent' },
        calendar1: { family: 'AntDesign', name: 'calendar' },
        map1: { family: 'Feather', name: 'map' },
        user1: { family: 'Feather', name: 'user' },
    });
    const { routingtemp } = React.useContext(TemplateRoutingContext);
    const [IsAnyBottomNavPageExists, setIsAnyBottomNavPageExists] = useState(false);
    const { fetch_inst_tabex_websitetemplatesQueryContext, StyleParseToIntFuncContext } = React.useContext(WebsiteDesignWorkPlaceContext);
        ...
        >
            <View
                style={{
                    ...
                    borderRadius: StyleParseToIntFuncContext(templateproperties_context.bottomnavborderradius),
                }}
            >
                {IsAnyBottomNavPageExists && (
                                    ...
                                    >
                                        {TabexIcons[arrayItem.mobapp_pageicon] != undefined && (
                                            ...
                                            >
                                                ...
                                                {arrayItem.pagename == 'Basket' && (
                                                    <View
                                                        style={[
                                                            generalstyles.allcentered,
                                                            { position: 'absolute', top: 0, right: -5, width: 15, height: 15, backgroundColor: 'orange', borderRadius: 100 },
                                                        ]}
                                                    >
                                                        <Text style={[generalstyles.poppinsMedium, { fontSize: 11, color: 'white' }]}>
