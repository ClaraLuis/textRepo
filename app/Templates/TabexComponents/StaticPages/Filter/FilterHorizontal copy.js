...
const FilterHorizontal = (props) => {
    ...
    useEffect(() => {
        ...
        var tempcat = [...instparentcollection];
                    ...
                    if (parent.isinftiler == 1 && parent.isshowntocustomers == 1) {
                        if (parent?.collections.length != 0) {
                            parent?.collections.forEach(function (collection, collectionindex) {
                                ...
                                //     parent.collections[collectionindex] = { Id: collection.collectionid, Name: langdetect == 'en' ? collection.title_en : collection.title_ar };
                                // } else {
                                // }
                                parent.collections[collectionindex] = { Id: collection.collectionid, Name: langdetect == 'en' ? collection.title_en : collection.title_ar };
                                if (collection.isinftiler != 1 && collection.isshowntocustomers != 1) {
                                    parent?.collections.splice(collectionindex, 1);
                                }
                            });
                            parent.collections.unshift({ Id: props.collid, Name: langdetect == 'en' ? 'All' : 'الكل' });
                            // alert(JSON.stringify(parent.collections));
                            // var sc = parent?.collections.findIndex((x) => x.collectionid === props.collid);
                            // alert(sc);
                        }
                        tempcat.push(parent);
                    }
